# -*- coding: utf-8 -*-
from odoo import models, fields, api, _
from odoo.exceptions import ValidationError
import requests
import base64

class ProductCategory(models.Model):
    _inherit = 'product.category'
   
    x_simpro_group_id = fields.Char(string="Simpro Group ID")


class Product(models.Model):
    _inherit = 'product.template'

    # Add a custom field to store Simpro contact ID
    x_simpro_product_id = fields.Char(string="Simpro Product ID")
    x_simpro_upc = fields.Char("UPC")
    x_simpro_vendore_part_number = fields.Char('Vendor Part Number')
    x_simpro_search_terms = fields.Char("Search Terms")
    x_simpro_is_favorite = fields.Boolean('Favorite')
    x_simpro_is_inventory = fields.Boolean('Inventory')
    x_simpro_is_assets = fields.Boolean('Assets')
    x_simpro_trade_price = fields.Float('Split Price')
    x_simpro_display_order = fields.Float('Display Order')
    x_simpro_manufacturer_id = fields.Many2one('product.manufacturer','Manufacturer')


#-----------------------------------------------------------------------------------------
    #get group of product means category
    def _cron_get_simpro_group(self):
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        # === STEP 1: Use the provided token directly ===
        if not access_token:
            raise ValidationError("Please Enter Access Token \nSetting > Simpro Integration")

        if not company_url:
            raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")
        
        # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'

        # === STEP 2: Call Simpro API ===
        group_url = f'{company_url}api/v1.0/companies/{company_id}/catalogGroups/'

        headers = {
            'Authorization': f'Bearer {access_token}',
            'Accept': 'application/json',
        }

        try:
            response = requests.get(group_url, headers=headers, timeout=30)
            if response.status_code != 200:
                raise Exception(f"Failed to get products: {response.text}")
            groups = response.json()
        except Exception as e:
            raise Exception(f"Group Fetch Error: {str(e)}")

        category = self.env['product.category']

        for group in groups:
            group_id = group.get('ID')
            specific_group_url = f'{company_url}api/v1.0/companies/{company_id}/catalogGroups/{group_id}'
            detail_res = requests.get(specific_group_url, headers=headers)
            if detail_res.status_code != 200:
                print(f"Failed to fetch Groups")
                continue
            data = detail_res.json() 

            parent_group = data.get('ParentGroup')
            existing_group = self.env['product.category'].search([('x_simpro_group_id', '=', group_id), ('name','=', data.get('Name'))], limit=1)
            if existing_group:
                print(f"Group Has already created:",existing_group)
                continue
            
            if parent_group:
                existing_parent_group = self.env['product.category'].search([('x_simpro_group_id', '=', parent_group.get('ID')), ('name','=', parent_group.get('Name'))], limit=1)
                if existing_parent_group:
                    parent_group_id = existing_parent_group.id
                if not existing_parent_group:
                    value = {
                        'name' : parent_group.get('Name'),
                        'x_simpro_group_id' : parent_group.get('ID'),
                        }
                    parent_category = category.create(value)
                    parent_group_id = parent_category.id
            
            values = {
                'name' : group.get('Name'),
                'parent_id' : parent_group_id if parent_group else False,
                'x_simpro_group_id' : group_id
                }
            categorys = category.create(values)
            print('\n Category Created',categorys)
#----------------------------------------------------------------------------------------------

#----------------------------------------------------------------------------------------------
#for product
    def _cron_get_simpro_product_data(self):
        pre_page_num = self.env.company.page_number
        company = self.env.company
        company.write({
            'page_number' : pre_page_num + 1
             })
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        # === STEP 1: Use the provided token directly ===
        # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'
        if not access_token:
            raise ValidationError("Please Enter Access Token \nSetting > Simpro Integration")

        if not company_url:
            raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")

        prod = self.env['product.template']
        categorys = self.env['product.category']
        vendor_parner = self.env['res.partner']
        miss_records = self.env['product.miss.records']
        manufacturers = self.env['product.manufacturer']
        
        # === STEP 2: Call Simpro API ===
        catalog_url = f'{company_url}api/v1.0/companies/{company_id}/catalogs/'

        headers = {
            'Authorization': f'Bearer {access_token}',
            'Accept': 'application/json',
        }

        params = {
            "pageSize": page_size,
            "page": self.env.company.page_number
            }

        #connect api and get data
        try:
            response = requests.get(catalog_url, headers=headers, timeout=30, params=params)
            if response.status_code != 200:
                raise Exception(f"Failed to get products: {response.text}")
            products = response.json()
        except Exception as e:
            raise Exception(f"Catalog Fetch Error: {str(e)}")

        #create product
        for product in products:
            product_id = product.get('ID')
            specific_product_url = f'{company_url}api/v1.0/companies/{company_id}/catalogs/{product_id}'
            detail_res = requests.get(specific_product_url, headers=headers)
            if detail_res.status_code != 200:
                print(f"Failed to fetch Product")
                continue

            data = detail_res.json() 
            # print("\n\n data",data)177
            name = data.get('Name') or data.get('PartNo')
            manufacturer = data.get('Manufacturer')
            mf_id = False
            if manufacturer:
                manufacturer_id = manufacturers.search([('name','=', manufacturer)], limit=1)
                if manufacturer_id:
                    mf_id = manufacturer_id.id
                else:
                    create_mf = manufacturers.create({
                        'name' : manufacturer
                        })
                    mf_id = create_mf.id

            if not name:
                print("No name or part number found, skipping.")
                mp = miss_records.search([('x_simpro_types','=','product'), ('simpro_id','=',data.get('ID'))])
                if not mp:
                    miss_record = miss_records.create({
                        'simpro_id' : data.get('ID'),
                        'product_price' : data.get('TradePrice'),
                        'x_simpro_types' : 'product',
                        })
                    print('\n Miss product log created', miss_record)
                continue

            existing = prod.search([('x_simpro_product_id', '=', data.get('ID')), ('default_code','=', data.get('PartNo'))], limit=1)
            if existing:
                if manufacturer:
                    print('\n x_simpro_manufacturer_id',existing.x_simpro_manufacturer_id)
                    existing.write({
                        'x_simpro_manufacturer_id': mf_id,
                        })
                print("Product Already Created",existing)
                continue

            vendors = data.get('Vendors')

            # Handle category
            category = data.get('Group')
            category_id = False
            if category:
                existing_category = categorys.search([
                    ('x_simpro_group_id', '=', category.get('ID')),
                    ('name', '=', category.get('Name'))
                ], limit=1)
                if existing_category:
                    category_id = existing_category.id
                else:
                    new_category = categorys.create({
                        'name': category.get('Name'),
                        'x_simpro_group_id': category.get('ID')
                    })
                    category_id = new_category.id
            
            values = {
                    'name' : name,
                    'x_simpro_product_id' : data.get('ID'),
                    'standard_price' : data.get('TradePrice'),
                    'default_code' : data.get('PartNo'),
                    'list_price' : data.get('SellPrice'),
                    'categ_id' : category_id if category_id else 1,
                    'x_simpro_upc' : data.get('UPC'),
                    'x_simpro_manufacturer_id' : mf_id,
                    'x_simpro_search_terms' : data.get('SearchTerm'),
                    'is_favorite' :  data.get('IsFavorite'),
                    'x_simpro_is_inventory' : data.get('IsInventory'),
                    'x_simpro_is_assets' : data.get('IsAsset'),
                    'x_simpro_trade_price' : data.get('SplitPrice'),
                    'x_simpro_display_order' : data.get('DisplayOrder'),
                }

            product_template =  prod.create(values)
            vendor_lines = []
            if vendors:
                for vendor in vendors:
                    vendor_id = vendor.get('Vendor')
                    v_id = vendor_id.get('ID')
                    odoo_vendor_id = vendor_parner.search([('x_simpro_contact_id','=',v_id), ('x_simpro_types','=','vendore')])
                    if not odoo_vendor_id:
                        continue
                    vendor_lines.append((0, 0, {
                            'partner_id' : odoo_vendor_id.id,
                            'product_tmpl_id' : product_template.id,
                            'discount' : vendor.get('Discount'),
                            'min_qty' : vendor.get('VendorQuantity'),
                            'price' : vendor.get('NettPrice'),
                        }))

            product_template.write({
                'seller_ids' : vendor_lines
                })
            print('\n Product Created',product_template)


class ResPartner(models.Model):
    _inherit = 'res.partner'

    # Add a custom field to store Simpro contact ID
    x_simpro_contact_id = fields.Char(string="Simpro ID", index=True)
    x_simpro_company_fax = fields.Char('Company Fax')
    x_simpro_types = fields.Selection([('vendore','Vendors'), ('contact','Contact'), ('customer','Customer'), ('contractor','Contractor')])
    # site_ids = fields.One2many('plc.sites','customer_id','Sites')
    # contact_site_ids = fields.One2many('plc.sites','contact_id','Sites')
    simpro_id = fields.Many2one('res.partner', domain=[('is_contact', '!=', False)], string='Primary Contact')
    site_id = fields.Many2one('res.partner', domain=[('is_sites', '!=', False)], string='Site')
    simpro_site_ids = fields.One2many('res.partner','simpro_id','Sites')
    is_customer = fields.Boolean('Is Customer')
    is_vendor = fields.Boolean('Is Vendor')
    is_contact = fields.Boolean('Is Contact')
    is_sites = fields.Boolean('Is Sites')
    is_contractor = fields.Boolean('Is Contractor')
    count_sites = fields.Integer('Helpdesk', compute='_compute_sites_count')

    # @api.onchange('site_id')
    # def _onchage_on_site_id(self):
    #     for rec in self:
    #         rec.name = rec.site_id.name
    #         rec.street = rec.site_id.street
    #         rec.city = rec.site_id.city
    #         rec.state_id = rec.state_id.id
    #         rec.zip = rec.site_id.zip


    def _compute_sites_count(self):
        for order in self:
            order.count_sites = self.env['plc.sites'].search_count([('contact_ids', 'in', self.ids)])

    def view_sites(self):
        self.ensure_one()
        sites_ids = self.env['plc.sites'].search([('contact_ids', 'in', self.id)]).ids
        action = {  
            'res_model': 'plc.sites',
            'type': 'ir.actions.act_window',
            'name': _("Sites for %s", self.name),
            'domain': [('id', 'in', sites_ids)],
            'view_mode': 'list,form'
        }
        return action

# ------------------inside code is for get data and uper side is for devloping----------------------------------------------------------------------------------------------
    def _create_site_conatct(self,odoo_site_id,simpro_site_id,company_url,company_id,headers):
        Contact = self.env['res.partner'].search([])
        url = f'{company_url}api/v1.0/companies/{company_id}/sites/{simpro_site_id}/contacts/'
        try:
            response = requests.get(url, headers=headers)
            if response.status_code != 200:
                raise Exception(f"Failed to get contacts: {response.text}")
            data = response.json()
        except Exception as e:
            raise Exception(f"Attachments Fetch Error: {str(e)}")
        for record in data:
            print('\n\n record',record)
            find_contact = Contact.search([('x_simpro_contact_id','=',record.get('ID')),('is_contact','!=',False)])
            if find_contact:
                find_contact.write({
                    'parent_id' : odoo_site_id.id
                    })

    def _create_vendor_conatct(self,odoo_vendor_id,simpro_vendor_id,company_url,company_id,headers):
        Contact = self.env['res.partner'].search([])
        url = f'{company_url}api/v1.0/companies/{company_id}/vendors/{simpro_vendor_id}/contacts/'
        try:
            response = requests.get(url, headers=headers)
            if response.status_code != 200:
                raise Exception(f"Failed to get contacts: {response.text}")
            data = response.json()
        except Exception as e:
            raise Exception(f"Attachments Fetch Error: {str(e)}")
        for record in data:
            print('\n\n record',record)
            find_contact = Contact.search([('x_simpro_contact_id','=',record.get('ID')),('is_contact','!=',False)])
            if find_contact:
                find_contact.write({
                    'parent_id' : odoo_vendor_id.id
                    })

    def mearge_conatct_with_customer(self,current_customer_id,contact_data):
        Contact = self.env['res.partner'].search([])
        current_customer_id = current_customer_id
        print('\n current_customer_id',current_customer_id)
        for record in contact_data:
            print('\n\n record',record)
            find_contact = Contact.search([('x_simpro_contact_id','=',record.get('ID')),('is_contact','!=',False)])
            if find_contact:
                find_contact.write({
                    'parent_id' : current_customer_id.id
                    })
    
    def get_attachments(self, external_id, headers, customer_id, value):
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        if value == 'customer':
            attachemenrt_url = f'{company_url}api/v1.0/companies/{company_id}/customers/{external_id}/attachments/files/'
        if value == 'vendors':
            attachemenrt_url = f'{company_url}api/v1.0/companies/{company_id}/vendors/{external_id}/attachments/files/'
        if value == 'contractor':
            attachemenrt_url = f'{company_url}api/v1.0/companies/{company_id}/contractors/{external_id}/attachments/files/'
        
        try:
            response = requests.get(attachemenrt_url, headers=headers)
            # print("\n\n response",response.headers)
            if response.status_code != 200:
                raise Exception(f"Failed to get contacts: {response.text}")
            attachments = response.json()
        except Exception as e:
            raise Exception(f"Attachments Fetch Error: {str(e)}")

        for attachment in attachments:
            attachment_id = attachment.get('ID')
            file_name = attachment.get('Filename')
            if value == 'customer':
                attachemenrt__ids = f'{company_url}api/v1.0/companies/{company_id}/customers/{external_id}/attachments/files/{attachment_id}?display=Base64'
            if value == 'vendors':
                attachemenrt__ids = f'{company_url}api/v1.0/companies/{company_id}/vendors/{external_id}/attachments/files/{attachment_id}?display=Base64'
            if value == 'contractor':
                attachemenrt__ids = f'{company_url}api/v1.0/companies/{company_id}/contractors/{external_id}/attachments/files/{attachment_id}?display=Base64'

            response = requests.get(attachemenrt__ids, headers=headers)

            if response.status_code != 200:
                print(f"Failed to download: {file_name}")
                continue

            result_json = response.json()
            base64_data = result_json.get('Base64Data')
            if not base64_data:
                print(f"No Base64Data for: {file_name}")
                continue

            new_attachment = self.env['ir.attachment'].create({
                'name': file_name,
                'type': 'binary',
                'datas': base64_data,
                'res_model': 'res.partner',
                'res_id': customer_id,
                'mimetype': 'application/pdf',
            })
            # print('\n New Attachment Created',new_attachment)

    #for create child address of customer
    def _create_customer_address(self, parent_id, billing_address):
        customer = self.env['res.partner']
        country = self.env['res.country'].search([("name",'=',billing_address.get('Country'))])
        state = self.env['res.country.state']
        state_id = False
        if country:
            state_id = state.search([('code','=',billing_address.get('State')), ('country_id','=',country.id)], limit=1)
        values = {
                'type' : 'invoice',
                'street' : billing_address.get('Address'),
                'city' : billing_address.get('City'),
                'zip' : billing_address.get('PostalCode'),
                'street' : billing_address.get('Address'),
                'country_id' : country.id if country else False,
                'state_id' : state_id.id if state_id else False,
                'parent_id' : parent_id,
                
            }
        parent_customer = customer.create(values)

#------------------------------------------------------------------------------------------
#customer
    def _cron_get_simpro_customer(self):
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        # === STEP 1: Use the provided token directly ===
        if not access_token:
            raise ValidationError("Please Enter Access Token \nSetting > Simpro Integration")

        if not company_url:
            raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")

        # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'
        country = self.env['res.country']
        customer = self.env['res.partner']
        state = self.env['res.country.state']
        miss_records = self.env['product.miss.records']
        term_line_obj = self.env['account.payment.term.line'].sudo()
        
        # === STEP 2: API endpoint for contacts ===
        contact_url = f'{company_url}api/v1.0/companies/{company_id}/customers/companies/'
        
        headers = {
            'Authorization': f'Bearer {access_token}',
            'Accept': 'application/json',
        }

        #this is for which page of simpro and how many conatact you will get
        params = {
            "pageSize": page_size,
            "page": page
            }

        try:
            response = requests.get(contact_url, headers=headers, params=params)
            if response.status_code != 200:
                raise Exception(f"Failed to get contacts: {response.text}")
            contacts = response.json()
        except Exception as e:
            raise Exception(f"Contact Fetch Error: {str(e)}")

        # === STEP 3: Create or Update Contacts in Odoo ===
        for contact in contacts:
            external_id = contact.get('ID')  # Simpro contact ID
            print('\n external_id',external_id)
            specific_contact_url = f'{company_url}api/v1.0/companies/{company_id}/customers/companies/{external_id}'
            detail_res = requests.get(specific_contact_url, headers=headers)

            if detail_res.status_code != 200:
                print(f"Failed to fetch customer")
                continue
            data = detail_res.json() 
            print('\n\n data ',data)
            
            addresh = data.get('Address')
            customer_name = contact.get('CompanyName')
            billing_address = data.get('BillingAddress')

            payment_term_id = None
            if data.get('Banking') and data.get('Banking').get('PaymentTerms'):
                payment_terms_data = data.get('Banking').get('PaymentTerms')
                delay_type = 'days_after'
                if payment_terms_data.get('Type') == 'EOM':
                    delay_type = 'days_after_end_of_month'
                payment_term_line = term_line_obj.search([
                    ('nb_days', '=', payment_terms_data.get('Days')),
                    ('delay_type', '=', delay_type),
                ], limit=1, order="id desc")
                if payment_term_line:
                    payment_term_id = payment_term_line.payment_id.id

            if not customer_name:
                cs = miss_records.search([('x_simpro_types','=','customer'), ('simpro_id','=',data.get('ID'))])
                if not cs:
                    miss_record = miss_records.create({
                        'simpro_id' : data.get('ID'),
                        'email' : data.get('Email'),
                        'x_simpro_types' : 'customer',
                        })
                    
                continue
            
            existing = self.env['res.partner'].search([('is_customer','!=',False),('x_simpro_contact_id','=', external_id)], limit=1)
            country_id = country.search([("name",'=',addresh.get('Country'))]) 
            state_id = state.search([('code','=',addresh.get('State')), ('country_id','=',country_id.id)], limit=1)
            contact_data = data.get('Contacts')

            values = {
                'is_customer' : 1,
                'name': data.get('CompanyName'),
                'email': data.get('email'),
                'phone': data.get('phone'),
                'street': addresh.get('Address'),
                'zip': addresh.get('PostalCode'),
                'city': addresh.get('City'),
                'x_simpro_company_fax': data.get('Fax'),
                'country_id' : country_id.id if country_id else False,
                'state_id' : state_id.id if state_id else False,
                'x_simpro_contact_id': external_id,
                'vat' : data.get('EIN'),
                'property_payment_term_id': payment_term_id
            }
            if existing:
                existing.sudo().write(values)
                if contact_data:
                    self.mearge_conatct_with_customer(existing,contact_data)
                print("Customer Already Created",existing)
                continue

            values.update({'company_type': 'company'})
            cus = customer.create(values)
            
            ##for create attachments of customer
            self.get_attachments(external_id,headers,cus.id,'customer')
            self.mearge_conatct_with_customer(cus, contact_data)
            ##create customer address
            if billing_address:
                self._create_customer_address(cus.id, billing_address)
            print('\n customer created',cus)

         
# -------------------------------------------------------------------------------
# vendors
    def _cron_get_simpro_vendors(self):
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        # === STEP 1: Use the provided token directly ===
        # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'
        if not access_token:
            raise ValidationError("Please Enter Access Token setting\n > Simpro Integration")

        if not company_url:
            raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")

        country = self.env['res.country']
        customer = self.env['res.partner']
        miss_records = self.env['product.miss.records']
        state = self.env['res.country.state']
        
        # === STEP 2: API endpoint for contacts ===
        contact_url = f'{company_url}api/v1.0/companies/{company_id}/vendors/'
        headers = {
            'Authorization': f'Bearer {access_token}',
            'Accept': 'application/json',
        }

        params = {
            "pageSize": page_size,
            "page": page
            }

        try:
            response = requests.get(contact_url, headers=headers, params=params)
            if response.status_code != 200:
                raise Exception(f"Failed to get contacts: {response.text}")
            contacts = response.json()
        except Exception as e:
            raise Exception(f"Vendors Fetch Error: {str(e)}")

        # # === STEP 3: Create or Update Contacts in Odoo ===
        for contact in contacts:
            external_id = contact.get('ID')  # Simpro contact ID
            specific_contact_url = f'{company_url}api/v1.0/companies/{company_id}/vendors/{external_id}'
            detail_res = requests.get(specific_contact_url, headers=headers)
            if detail_res.status_code != 200:
                print(f"Failed to fetch vendors")
                continue
            data = detail_res.json() 
            print('\n\n data',data)

            customer_name = contact.get('Name')
            addresh = data.get('Address')
            billing_address = data.get('BillingAddress')
            
            if not customer_name:
                vn = miss_records.search([('x_simpro_types','=','vendore'), ('simpro_id','=', data.get('ID'))])
                if not vn:
                    miss_record = miss_records.create({
                        'simpro_id' : data.get('ID'),
                        'email' : data.get('Email'),
                        'x_simpro_types' : 'vendore',
                        })
                    print('\nRecord was skiped ann log created',miss_record)
                continue
            
            existing = self.env['res.partner'].search([('is_vendor','!=',False),('x_simpro_contact_id','=', external_id)], limit=1)
            country_id = country.search([("name",'=',addresh.get('Country'))]) 
            contact_data = data.get('Contacts')

            if existing:
                self._create_vendor_conatct(existing,external_id,company_url,company_id,headers)
                print("Vendors Already Created",existing)
                continue
            state_id = state.search([('code','=',addresh.get('State')), ('country_id','=',country_id.id)], limit=1)
            values = {
                'x_simpro_types': 'vendore',
                'is_vendor': 1,
                'company_type' : 'company',
                'name': contact.get('Name'),
                'email': data.get('Email'),
                'phone': data.get('Phone'),
                'street': addresh.get('Address'),
                'zip': addresh.get('PostalCode'),
                'city': addresh.get('City'),
                'x_simpro_company_fax': data.get('Fax'),
                'country_id' : country_id.id if country_id else False,
                'state_id' : state_id.id if state_id else False,
                'x_simpro_contact_id': external_id,
                'vat' : data.get('EIN')
            }

            vendors = customer.create(values)
            print('\n vendor created',vendors)
            self._create_vendor_conatct(vendors,external_id,company_url,company_id,headers)
            # for create attachments of vendor
            self.get_attachments(external_id,headers,vendors.id,'vendors')

            #create customer address
            if billing_address:
                self._create_customer_address(vendors.id, billing_address)


    #this script is for contact
    def _cron_get_simpro_contact(self):
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        # === STEP 1: Use the provided token directly ===
        # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'
        if not access_token:
            raise ValidationError("Please Enter Access Token \nSetting > Simpro Integration")

        if not company_url:
            raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")

        country = self.env['res.country']
        customer = self.env['res.partner']
        miss_records = self.env['product.miss.records']
        
        # === STEP 2: API endpoint for contacts ===
        contact_url = f'{company_url}api/v1.0/companies/{company_id}/contacts/'
        headers = {
            'Authorization': f'Bearer {access_token}',
            'Accept': 'application/json',
        }

        params = {
            "pageSize": page_size,
            "page": page
            }

        try:
            response = requests.get(contact_url, headers=headers, params=params)
            if response.status_code != 200:
                raise Exception(f"Failed to get contacts: {response.text}")
            contacts = response.json()

        except Exception as e:
            raise Exception(f"Contact Fetch Error: {str(e)}")

        for contact in contacts:
            external_id = contact.get('ID')  # Simpro contact ID
            specific_contact_url = f'{company_url}api/v1.0/companies/{company_id}/contacts/{external_id}'
            detail_res = requests.get(specific_contact_url, headers=headers)
            if detail_res.status_code != 200:
                print(f"Failed to fetch customer")
                continue
            data = detail_res.json() 
            
            customer_name = contact.get('GivenName')
        
            if not customer_name:
                cn = miss_records.search([('x_simpro_types','=','contact'), ('simpro_id','=',data.get('ID'))])
                if not cn:
                    miss_record = miss_records.create({
                        'simpro_id' : data.get('ID'),
                        'email' : data.get('Email'),
                        'x_simpro_types' : 'contact',
                        })
                    print('\nRecord was skiped ann log created',miss_record)
                continue

            values = {
                'is_contact': 1,
                'name': f'{data.get("GivenName")} {data.get("FamilyName")}',
                'email': data.get('Email'),
                'phone': data.get('CellPhone'),
                'mobile': data.get('WorkPhone'),
                'comment': data.get('Notes'),
                'function': data.get('Position'),
                'x_simpro_company_fax': data.get('Fax'),
                'x_simpro_contact_id': external_id,
            }
            existing = self.env['res.partner'].search([('is_contact','!=',False), ('x_simpro_contact_id','=', external_id)], limit=1)
            
            if existing:
                existing.sudo().write(values)
                print("Contact Already Created",existing)
                continue

            values.update({'company_type': 'company'})
            contact = customer.create(values)
            print('\n Contact created', contact)

    # def _cron_get_simpro_sites(self):
    #     company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
    #     company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
    #     access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
    #     page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
    #     page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

    #     # === STEP 1: Use the provided token directly ===
    #     # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'
    #     if not access_token:
    #         raise ValidationError("Please Enter Access Token \nSetting > Simpro Integration")

    #     if not company_url:
    #         raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")

    #     country = self.env['res.country']
    #     new_sites = self.env['res.partner']
    #     miss_records = self.env['product.miss.records']
    #     state = self.env['res.country.state']
        
    #     # === STEP 2: API endpoint for Sites ===
    #     site_url = f'{company_url}api/v1.0/companies/{company_id}/sites/'
    #     headers = {
    #         'Authorization': f'Bearer {access_token}',
    #         'Accept': 'application/json',
    #     }

    #     params = {
    #         "pageSize": page_size,
    #         "page": page
    #         }

    #     try:
    #         response = requests.get(site_url, headers=headers, params=params)
    #         if response.status_code != 200:
    #             raise Exception(f"Failed to get contacts: {response.text}")
    #         sites = response.json()
    #     except Exception as e:
    #         raise Exception(f"Sites Fetch Error: {str(e)}")

    #     for site in sites:
    #         site_id = site.get('ID')
    #         specific_site_url = f'{company_url}api/v1.0/companies/{company_id}/sites/{site_id}'
    #         detail_res = requests.get(specific_site_url, headers=headers)
    #         if detail_res.status_code != 200:
    #             print(f"Failed to fetch Sites")
    #             continue
    #         data = detail_res.json() 

    #         site_name = data.get('Name')

    #         if not site_name:
    #             st = miss_records.search([('x_simpro_types','=','site'), ('simpro_id','=',data.get('ID'))])
    #             if not st:
    #                 miss_record = miss_records.create({
    #                     'simpro_id' : data.get('ID'),
    #                     'site_zip_code' : data.get('PostalCode'),
    #                     'x_simpro_types' : 'site',
    #                     })
    #                 print('\nRecord was skiped ann log created',miss_record)
    #             continue

            

    #         existing = self.env['res.partner'].search([('x_simpro_contact_id', '=', site_id), ('is_sites','!=', False)], limit=1)
    #         if existing:
    #             self._create_site_conatct(existing,site_id,company_url,company_id,headers)
    #             print("Site Already Created",existing)
    #             continue
    #         billing_address = data.get('BillingAddress')
    #         addresh = data.get('Address')
    #         country_id = country.search([("name",'=',addresh.get('Country'))])
    #         primary_contact = data.get('PrimaryContact')
    #         primary_contact_id = primary_contact.get('Contact')
    #         state_id = state.search([('code','=',addresh.get('State')), ('country_id','=',country_id.id)], limit=1)
    #         values = {
    #             'is_sites': 1,
    #             'company_type' : 'company',
    #             'name': data.get('Name'),
    #             'street': addresh.get('Address'),
    #             'zip': addresh.get('PostalCode'),
    #             'city': addresh.get('City'),
    #             'country_id' : country_id.id if country_id else False,
    #             'state_id' : state_id.id if state_id else False,
    #             'x_simpro_contact_id': site_id,
    #         }

    #         odoo_site = new_sites.create(values)
    #         print('\n Created new site', odoo_site)
    #         self._create_site_conatct(odoo_site,site_id,company_url,company_id,headers)
    #         if primary_contact_id:
    #             existing_contact = new_sites.search([('x_simpro_contact_id', '=', primary_contact_id.get('ID')), ('is_contact','!=', False), ('email','=', primary_contact_id.get('Email'))], limit=1)
    #             if existing_contact:
    #                 odoo_site.write({
    #                     'simpro_id' : existing_contact.id
    #                     })

    #         # create customer address
    #         if billing_address.get('Address') or billing_address.get('City'):
    #             self._create_customer_address(odoo_site.id, billing_address)

#-----------------------------------------------------------------------------------------------
#this is for contractor ------------------------------------------------------------------------

    #create primary contact for contractor
    def _create_primary_contact(self, parent_id, primary_contact):
        customer = self.env['res.partner']
        values = {
                    'type' : 'other',
                    'email' : primary_contact.get('Email'),
                    'phone' : primary_contact.get('WorkPhone'),
                    'mobile' : primary_contact.get('CellPhone'),
                    'parent_id' : parent_id           
                }
        primary_contact = customer.create(values)

    def _cron_get_simpro_contractor(self):
        company_url = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company_url')
        company_id = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.company') or 0
        access_token = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.access_token')
        page_size = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page_size')
        page = self.env['ir.config_parameter'].sudo().get_param('ehcs_simpro_integration.page')

        # access_token = '37a421022e74dd11d9cfc1f000f9c6d88f8d8c2b'
   
        country = self.env['res.country']
        create_contractors = self.env['res.partner']
        state = self.env['res.country.state']

        if not access_token:
            raise ValidationError("Please Enter Access Token \nSetting > Simpro Integration")

        if not company_url:
            raise ValidationError("Please Set Your Company Simpro Url\n setting > Simpro Integration")  

        contractor_url = f'{company_url}api/v1.0/companies/{company_id}/contractors/'
        
        headers = {
            'Authorization': f'Bearer {access_token}',
            'Accept': 'application/json',
        }
        
        # pagesize means how many records you will get and page mean which page of simpro
        params = {
            "pageSize": page_size,
            "page": page
            }

        try:
            response = requests.get(contractor_url, headers=headers, params=params)
            if response.status_code != 200:
                raise Exception(f"Failed to get Contractor: {response.text}")
            contractors = response.json()
        except Exception as e:
            raise Exception(f"Contractor Fetch Error: {str(e)}")

        # # === STEP 3: Create or Update Contractor in Odoo ===
        for contractor in contractors:
            external_id = contractor.get('ID')
            specific_contractor_url = f'{company_url}api/v1.0/companies/{company_id}/contractors/{external_id}'
            detail_res = requests.get(specific_contractor_url, headers=headers)
            
            if detail_res.status_code != 200:
                print(f"Failed to fetch Contractors")
                continue
            data = detail_res.json() 

            print('\n\n data',data)
            primary_contact = data.get('PrimaryContact')
            addresh = data.get('Address')
            country_id = country.search([("name",'=',addresh.get('Country'))])
            
            existing = self.env['res.partner'].search([('x_simpro_contact_id', '=', external_id), ('is_contractor','!=', False), ('name', '=', data.get('Name'))], limit=1)

            if existing:
                # if addresh.get('State') and country_id:
                #     find_state_id = state.search([('code','=',addresh.get('State')), ('country_id','=',country_id.id)])
                #     if not find_state_id:
                #         find_state_id = state.search([('name', '=', addresh.get('State')),('country_id', '=', country_id.id)], limit=1)
                #     if find_state_id:
                #         existing.write({
                #             'state_id' : find_state_id.id
                #             })
                print("Contractor Already Created",existing)
                continue

            state_id = state.search([('code','=',addresh.get('State')), ('country_id','=',country_id.id)], limit=1)
            values = {
                'is_contractor' : 1,
                'company_type' : 'company',
                'name': data.get('Name'),
                'street': addresh.get('Address'),
                'zip': addresh.get('PostalCode'),
                'city': addresh.get('City'),
                'country_id' : country_id.id if country_id else False,
                'state_id' : state_id.id if state_id else False,
                'x_simpro_contact_id': external_id,
                'vat' : data.get('EIN')
            }

            new_contractor = create_contractors.create(values)
            ## for create attachments of vendor
            self.get_attachments(external_id,headers,new_contractor.id,'contractor')

            ## create customer address
            if primary_contact.get('Email'):
                self._create_primary_contact(new_contractor.id, primary_contact)
        